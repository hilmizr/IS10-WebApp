...
class CompanyProfileController extends Controller
{
    public function edit(Request $request): View
    {
        ...
        if ($request->user()->phone != null){
            $request->user()->phone = $this->DecryptAES($request->user()->phone, $request->user()->companyKey->key);
        }
        if ($request->user()->name != null){
            $request->user()->name = $this->DecryptAES($request->user()->name, $request->user()->companyKey->key);
        }
        if ($request->user()->university != null){
            $request->user()->university = $this->DecryptAES($request->user()->university, $request->user()->companyKey->key);
        }
        if ($request->user()->date_of_birth != null){
            $request->user()->date_of_birth = $this->DecryptAES($request->user()->date_of_birth, $request->user()->companyKey->key);
        }
        if ($request->user()->id_number != null){
            $request->user()->id_number = $this->DecryptAES($request->user()->id_number, $request->user()->companyKey->key);
        }
        if ($request->user()->student_id_number != null){
            $request->user()->student_id_number = $this->DecryptAES($request->user()->student_id_number, $request->user()->companyKey->key);
        }
        if ($request->user()->major != null){
            $request->user()->major = $this->DecryptAES($request->user()->major, $request->user()->companyKey->key);
        }
    ...
    public function update(ProfileUpdateRequest $request): RedirectResponse
    {
        ...
        if ($request->user()->isDirty('email')) {
            ...
            $request->user()->email = $this->EncryptAES($request->user()->email, $request->user()->companyKey->key);
        }
        if ($request->user()->isDirty('address')) {
            $request->user()->address = $this->EncryptAES($request->user()->address, $request->user()->companyKey->key);
        }
        if ($request->user()->isDirty('phone')) {
            $request->user()->phone = $this->EncryptAES($request->user()->phone, $request->user()->companyKey->key);
        }
        if ($request->user()->isDirty('name')) {
            $request->user()->name = $this->EncryptAES($request->user()->name, $request->user()->companyKey->key);
        }
        if ($request->user()->isDirty('university')) {
            $request->user()->university = $this->EncryptAES($request->user()->university, $request->user()->companyKey->key);
        }
        if ($request->user()->isDirty('date_of_birth')) {
            $request->user()->date_of_birth = $this->EncryptAES($request->user()->date_of_birth, $request->user()->companyKey->key);
        }
        if ($request->user()->isDirty('id_number')) {
            $request->user()->id_number = $this->EncryptAES($request->user()->id_number, $request->user()->companyKey->key);
        }
        if ($request->user()->isDirty('student_id_number')) {
            $request->user()->student_id_number = $this->EncryptAES($request->user()->student_id_number, $request->user()->companyKey->key);
        }
        if ($request->user()->isDirty('major')) {
            $request->user()->major = $this->EncryptAES($request->user()->major, $request->user()->companyKey->key);
        }
